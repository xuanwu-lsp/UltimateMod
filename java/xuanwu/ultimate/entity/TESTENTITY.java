  ...
  implements IThrowableEntity
{
  ...
  public TESTENTITY(World par1World, EntityLivingBase entityLiving, float AttackLevel, boolean multiHit)
  {
    this(par1World, entityLiving, AttackLevel);
    setIsSingleHit(multiHit);
  }
  
  public TESTENTITY(World par1World, EntityLivingBase entityLiving, float AttackLevel)
  {
    this(par1World);
  ...
  {
    return dead ? getDataWatcher().getWatchableObjectInt(3) : Integer.MAX_VALUE;
  }
  ...
  public void onUpdate()
  {
    ...
    ultimateCore.DestoryRangeBlock(this.worldObj,(int)Math.round(this.posX),(int)Math.round(this.posY),(int)Math.round(this.posZ),3);
    	}
    }
      if ((this.ticksExisted < 8) && (this.ticksExisted % 2 == 0)) {
        playSound("mob.wither.hurt", 0.2F, 0.5F + 0.25F * this.rand.nextFloat());
      }
      List<Entity> entities = ultimateCore.getSurroundingEntities(this,1);
...
if(entity instanceof EntityLivingBase) {
	...
	if(entity.getEntityData().getBoolean("ImmuneToRailGun")) {
		continue;
	}
	entity.attackEntityFrom(Register.UDS(),0);
	((EntityLivingBase) entity).setHealth(((EntityLivingBase) entity).getHealth()-100);
}
    	  }
      }
      if(dead) {
